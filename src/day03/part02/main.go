...
func getPaths(moves []string) []Line {
	...
	for _, move := range moves {
		...
		newLine.steps = lastLine.steps + pos
	...
	for _, wire1Path := range wire1 {
		for _, wire2Path := range wire2 {
			...
			if newIntersect.x != 0 || newIntersect.y != 0 {
				fmt.Println(`intersection`, newIntersect, wire1Path, wire2Path)
				fmt.Println(`total steps`, wire1Path.steps+wire2Path.steps)
...
func main() {
	...
	var Wire1 []string = strings.Split("R75,D30,R83,U83,L12,D49,R71,U7,L72", ",")
	var Wire2 []string = strings.Split("U62,R66,U55,R34,D71,R55,D58,R83", ",")

	// var Wire1 []string = strings.Split("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51", ",")
	// var Wire2 []string = strings.Split("U98,R91,D20,R16,D67,R40,U7,R15,U6,R7", ",")
